...
func main() {
	...
	filehash := "16836a08292adbd00f549e2b5050a3fe41471598"

	// 1. 请求初始化分块上传接口
	resp, err := http.PostForm(
		"http://localhost:8081/file/mpupload/init",
		url.Values{
			"username": {username},
			"token":    {token},
			"filehash": {filehash},
	...
	// 3. 请求分块上传接口
	filename := "/data/pkg/sogoupinyin_2.3.2.07_amd64-831.deb"
	tURL := "http://localhost:8081/file/mpupload/uppart?" +
		"username=admin&token=" + token + "&uploadid=" + uploadID
	multipartUpload(filename, tURL, chunkSize)

	// 4. 请求分块完成接口
	resp, err = http.PostForm(
		"http://localhost:8081/file/mpupload/complete",
		url.Values{
			"username": {username},
			"token":    {token},
			"filehash": {filehash},
			"filesize": {"47741076"},
			"filename": {"sogoupinyin_2.3.2.07_amd64-831.deb"},
			"uploadid": {uploadID},
		})
